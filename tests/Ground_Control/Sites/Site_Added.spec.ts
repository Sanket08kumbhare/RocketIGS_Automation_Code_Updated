...
test('Create a new site and verify it appears in the list of Sites', async({page, loginAsCompanyUser})=>{
    ...
    await page.getByPlaceholder('Enter Site Name').fill('Union Square MTA Station');
    await page.getByPlaceholder('Enter name of Contact Person').fill('Simon Tiffany');
    await page.getByPlaceholder('Enter Email Address').fill('tiffany.simon@mailinator.com');
    await page.getByPlaceholder('Enter Phone Number').fill('6495213011');
